#' Download of reference evapotranspiration (etp) and rainfall (ppt) from 'TerraClimate'
#'
#'
#' @description This function will download the ETp and ppt and will load a rasterstack according to the the region of interest (Region and sub_region).
#' @param variable variable to download. This function will download the etp or ppt (Rasterstack).
...
#' \dontrun{
#'
#' ### Downloading etp based on Brazil states.
#' see_brazil_states()
#'
#' img<-download_terraclimate(variable = "etp",
#'                           years = c(2018:2019),
#'                           region = "brazil",
#'                           sub_region = 13)
#'
...
#'                           sub_region = 6)
#' }
#' @return Load the ppt (Rainfall) or etp (reference evapotranspiration) rasterstack
#' @references
...
download_terraclimate <- function(variable, years, region, sub_region){
  ...
  if(variable == "ppt"| variable == "etp"){
    ...
    return(s)
  } else {message("Please, pay attention. You should download the variable ppt or etp!")}
}
